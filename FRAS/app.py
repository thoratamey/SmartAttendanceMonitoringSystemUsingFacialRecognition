...
def identify_face(facearray):
    model = joblib.load('static/knn_model_160x160.pkl')
...
def train_model():
    ...
    for user in userlist:
        for imgname in os.listdir(f'faces/{user}'):
            img = cv2.imread(f'faces/{user}/{imgname}')
    ...
    joblib.dump(knn,'static/knn_model_160x160.pkl')
...
def start():
    ...
    if 'knn_model_160x160.pkl' not in os.listdir('static'):
        names, rolls, times, l = extract_attendance()
...
def add():
    ...
    userimagefolder = 'faces/'+newusername+'_'+str(newuserid)
